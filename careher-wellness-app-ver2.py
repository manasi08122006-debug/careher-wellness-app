...
Remedies = {
    "Menstrual": {
        "Cramps": {
            "title": "To ease pain ",
            "exercise": [
                ("Child's Pose (Balasana)","Relaxes lower back and reduces cramp intensity.", r"C:\Users\Manasi\Downloads\WhatsApp Image 2026-04-23 at 5.22.09 PM.jpeg"),
                ("Knees-to-Chest (Apanasana)","Gently massages abdomen and eases pain.",r"C:\Users\Manasi\Downloads\WhatsApp Image 2026-04-23 at 5.22.10 PM (1).jpeg"),
                ("Supine Twist","Releases tension in lower body.",r"C:\Users\Manasi\Downloads\WhatsApp Image 2026-04-23 at 5.22.11 PM.jpeg"),
                ("Legs Up the Wall","Improves blood flow and relaxation.",r"C:\Users\Manasi\Downloads\WhatsApp Image 2026-04-23 at 5.22.10 PM.jpeg"),
                ("Deep Breathing","Calms nervous system and reduces pain perception.", r"D:\New folder\pic4.jpeg"),
            ],
            "food": [
...
def load_image(path, size=(220, 160)):
    try:
        key = (path, size)
        if key not in _image_cache:
            img = Image.open(path).convert("RGBA")
            img = img.resize(size, Image.LANCZOS)
            _image_cache[key] = ImageTk.PhotoImage(img)
...
# -────────────────────────────────────────────────────────────────────────────
#  REMEDIES - DETAIL PAGE  (IMAGE FIX APPLIED HERE)
# -────────────────────────────────────────────────────────────────────────────
def open_remedy_detail(condition_key):
    ...
    # ── FIXED card() - now accepts and displays img_path ───────────────────
    def card(parent, title, desc, bg="#FFF0F3", img_path=None):
        ...
        if img_path:
            photo = load_image(img_path, size=(220, 160))
            if photo:
                img_label = tk.Label(f, image=photo, bg=bg)
                img_label.image = photo   # prevent garbage collection
                img_label.pack(anchor="w", padx=10, pady=(2, 8))
            else:
                tk.Label(f, text="[Image not found]", font=("Arial", 9, "italic"),
                         bg=bg, fg="#aaaaaa").pack(anchor="w", padx=10, pady=(2, 8))
        else:
    ...
    for exercise_item in data["exercise"]:
        ...
        img_path = exercise_item[2] if len(exercise_item) == 3 else None
        card(inner, name, desc, bg="#FFF0F3", img_path=img_path)   # ← img_path passed
...
def open_predictor():
    ...
    def predict_phase():
        ...
        days_from_last = (today_date - last_period).days
